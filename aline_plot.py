...
data= read_file_time("2023-07-21-16-30_running04-09.xlsx",1,11) # read from line 2 to line 11
...
class Read_data_runing:
    ...
    def estimate_the_start_frame(self,skiprow=3):
        ...
        self.data_frame,index_read_frame = read_csv_from_frame(self.file_path)     
       
        self.Header1=read_specific_rows(self.file_path,index_read_frame)
        # self.Header2=read_specific_rows(self.file_path,index_read_frame)
...
data_run_left=pd.read_csv("2023-07-21-16-30_running04-09__left_leg_processed__.csv")
...
data_run_right=pd.read_csv("2023-07-21-16-30_running04-09__right_leg_processed__.csv")
...
def read_data(different_time_,list_file,List_column_to_read):
    ...
    for index,file_path in enumerate(list_file):
        ...
        # print(data_frame_file,"in this ooooooooooooooooooooooo")
...
def plot_average_with_error(interp_data_cycle,split_cycle,title1):
    ...
    # Adjust the y-coordinate to move the labels higher up
    bottom = min(0,np.min(interp_data_cycle))
    peak=np.max(interp_data_cycle)
    if bottom==0 :
        y_offset=bottom-(peak-bottom)/10
    else :
        y_offset=bottom-(peak-bottom)/10
    for i, label in enumerate(phase_labels):
        if i > 0:
            plt.text(average_split_cycle[i - 1], y_offset, label, color='red', ha='left')
        else:
    ...
    # minor_custom_xticks = np.arange(10, 101, 10)
...
different_time_from03=different_time
# print(different_time_from03)
